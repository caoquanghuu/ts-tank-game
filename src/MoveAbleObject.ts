...
export class MoveAbleObject {
  ...
  public _move(deltaTime: number) {
    ...
    const nextY =
      Number.parseInt(this._image.style.top) +
      ((this._speed * deltaTime) / 1000) * direction.y;
    const nextX =
      Number.parseInt(this._image.style.left) +
      ((this._speed * deltaTime) / 1000) * direction.x;
